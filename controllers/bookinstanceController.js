...
exports.bookinstance_delete_get = async function(req, res, next) {
  try {
    let bookinstance = await BookInstance.findById(req.params.id);
...
exports.bookinstance_delete_post = async function(req, res, next) {
  await BookInstance.findByIdAndRemove(req.body.bookinstanceid, function deleteGenre(err) {
    if (err) return next(err);
...
exports.bookinstance_update_get = function(req, res, next) {
  //let bookinstance =  BookInstance.findById(req.params.id);
  async.parallel({
    bookinstance: (callback) => BookInstance.findById(req.params.id).populate('book').exec(callback),
    book: (callback) => Book.find(callback),
  }, (err, results) => {
    if (err) return next(err);
    res.render('bookinstance_update_form', {title: 'Update Book Instance', bookinstance: results.bookinstance, books: results.book})
  })
};
